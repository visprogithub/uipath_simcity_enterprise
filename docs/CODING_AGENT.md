...
The Coding Agent is a feature in Maestro City that uses OpenAI GPT-4o to dynamically
generate complete, valid UiPath XAML workflow files based on the real-time state of your
simulation. Instead of starting from a blank Studio canvas, the Coding Agent inspects
...
4. Backend constructs a specialised XAML-generation prompt combining:
   ...
   • A requirement to include UiPath Action Center integration points
        │
        ▼
5. Backend sends the prompt to the OpenAI API (gpt-4.1-mini)
   via the OpenAI Python SDK
        │
        ▼
6. GPT-4o returns a complete, valid XAML workflow tailored to the
...
If `OPENAI_API_KEY` is set correctly, the response `badge` field will be
`"GENERATED BY CODING AGENT"` and `generatedBy` will contain `"gpt-4.1-mini"`.

If the key is missing, `badge` will be `"DEMO - No API Key Set"` and you will still
receive a valid, realistic mock XAML that can be imported into Studio.
...
```json
{
  "generatedBy": "OpenAI gpt-4.1-mini Coding Agent",
...
types) using GPT-4o. Useful for extending the simulation with domain-specific entities.
...
Diagnoses a faulted UiPath workflow using GPT-4o and returns a root cause analysis,
...
<Activity mc:Ignorable="sap sap2010 sap2020"
  ...
  <Sequence DisplayName="Incident Escalation — Crisis Context (Generated)">
    ...
    <!-- Generator: OpenAI gpt-4.1-mini | tick=42 | stability=28.4 -->
