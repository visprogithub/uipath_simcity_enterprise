...
Uses the Anthropic Claude API to dynamically generate UiPath XAML workflows,
...
async def generate_workflow(request: GenerateWorkflowRequest) -> Dict[str, Any]:
    ...
    if api_key:
        try:
            ...
            xaml = message.content[0].text.strip()
            badge = "GENERATED BY CODING AGENT"
            model_used = "claude-sonnet-4-6"
            logger.info(f"Claude generated XAML for process_type={process_type}, {len(xaml)} chars")
    ...
    return {
        "generatedBy": f"Claude {model_used} Coding Agent",
...
async def generate_entities(request: GenerateEntitiesRequest) -> Dict[str, Any]:
    """
    Generate simulation entity definitions using Claude AI.
    ...
    if api_key:
        try:
            ...
            badge = "GENERATED BY CODING AGENT"
            model_used = "claude-sonnet-4-6"
            logger.info(f"Claude generated {len(entities)} entities of type {request.entityType}")
        except Exception as e:
            logger.warning(f"Anthropic API call failed for entity generation, using mock: {e}")
...
async def debug_workflow(request: DebugWorkflowRequest) -> Dict[str, Any]:
    """
    Diagnose a faulted UiPath workflow using Claude AI and return a suggested fix.
    """
    start_time = time.time()
    sim_ctx = _build_simulation_context()
    ...
    if api_key:
        try:
            ...
            model_used = "claude-sonnet-4-6"
            logger.info(f"Claude debugged workflow {request.workflow_id}")
        except Exception as e:
            logger.warning(f"Anthropic API call failed for debug, using mock: {e}")
