...
Uses the OpenAI API (gpt-4.1-mini) to dynamically generate UiPath XAML workflows,
...
async def generate_entities(request: GenerateEntitiesRequest) -> Dict[str, Any]:
    """
    Generate simulation entity definitions using OpenAI gpt-4.1-mini.
...
async def debug_workflow(request: DebugWorkflowRequest) -> Dict[str, Any]:
    """
    Diagnose a faulted UiPath workflow using OpenAI gpt-4.1-mini and return a suggested fix.
    """
    start_time = time.time()
    sim_ctx = _build_simulation_context()
